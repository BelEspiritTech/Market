...
public class ExcelDataHelper {
	...
	public void pushEnCodes(BasicExcelData data){
		...
		List<IEssotEntity> enCodesList = daoFactory.getDAOClass(blank).getFilteredListOnPrimarKey(skuList);
		boolean isExist = false;
		
		if(enCodesList != null && !enCodesList.isEmpty()){
			for(IEssotEntity enCode : enCodesList ) {
				if(blank.getSkuName().equalsIgnoreCase(((ProductXENCode)enCode).getSkuName())){
					isExist = true;
					break;
				}				
			}
	...
	public void pushTechSpecs(BasicExcelData data){
		...
		List<IEssotEntity> techSpecList = daoFactory.getDAOClass(blank).getFilteredListOnPrimarKey(skuList);
		boolean isExist = false;
		
		if(techSpecList != null && !techSpecList.isEmpty()){
			for(IEssotEntity techSpec : techSpecList ) {
				if(blank.getSkuName().equalsIgnoreCase(((ProductXTechSpec)techSpec).getSkuName())){
					isExist = true;
					break;
				}				
			}
